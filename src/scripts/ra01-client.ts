type PaymentMethod = 'webpay' | 'transfer';
...
const allowedEvents=new Set(['ra01_landing_view','ra01_demo_view','ra01_registration_start','ra01_registration_submit','ra01_webpay_click','ra01_transfer_select','ra01_transfer_copy','ra01_payment_help','ra01_waitlist','ra01_professional_profile_click','ra01_materials_open','ra01_technical_check_complete']);
...
function track(event:string,detail:Partial<SafeEvent>={}){
 ...
 if(['webpay','transfer'].includes(detail.method??''))payload.method=detail.method;
...
if(previewForm){let busy=false;let started=false;
 ...
 safeStore.set('ra01.preview.registration',JSON.stringify({id,method:method.value==='transfer'?'transfer':'webpay',termsVersion:previewForm.dataset.termsVersion,acceptedAt:new Date().toISOString()}));
...
if(payment){let method:PaymentMethod='webpay';try{const r=JSON.parse(safeStore.get('ra01.preview.registration')??'{}');if(r.method==='transfer')method='transfer';const ref=document.querySelector('[data-preview-reference]');if(ref&&/^preview-[a-f0-9-]{36}$/.test(r.id??''))ref.textContent=r.id;}catch{/* keep safe default */}
 ...
 renderMethod(method);document.querySelectorAll<HTMLButtonElement>('[data-select-method]').forEach(b=>b.addEventListener('click',()=>{renderMethod(b.dataset.selectMethod==='transfer'?'transfer':'webpay');if(method==='transfer')track('ra01_transfer_select',{method})}));
 ...
 document.querySelectorAll<HTMLButtonElement>('[data-preview-outcome]').forEach(b=>b.addEventListener('click',()=>{const out=document.querySelector<HTMLElement>('[data-payment-status]')!;const key=b.dataset.previewOutcome;out.textContent=key==='approved'?'Simulación: Webpay informa pago aprobado. La inscripción sigue pendiente de verificación; esta prueba no reserva un cupo.':key==='rejected'?'Simulación: pago rechazado. Puedes revisar tus datos o elegir transferencia. Si ves un cargo, no repitas el pago hasta consultar.':'Simulación: pago cancelado. No se ha confirmado ninguna inscripción.';out.focus();}));
